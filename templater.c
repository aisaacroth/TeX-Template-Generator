...
int main(int argc, char **argv) {
    ...
    fileName = malloc(strlen(seriesType) + 1 + strlen(seriesNumber) + strlen(extension));
    ...
    if (strstr(classNumber, "COMS") || strstr(classNumber, "CSEE")) {
        ...
        fprintf(texFile, "\\title{%s \\\\ %s \\\\ %s \\#%s}\n", classNumber, className, seriesType, seriesNumber);
    ...
    printf("%s, %s, %s, %s\n", className, seriesType, classNumber, seriesNumber);
    fclose(texFile);
    return 0;
}
